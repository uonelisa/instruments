...
class MCL_LiData_DataReadings:
    ...
    generalreadings_nt = namedtuple('MCL_LIData_generalreadings', 'dt_s cyclespersample syncindex time_s lockinf_hz pll1_hz pll2_hz composite1_hz')
    ...
    def __init__(self, lockinset):
        ...
        r = []
        thetadeg = []
        self._datareadings = self.nt(generalreadings = generalreadings, moduledata = moduledata, dc = dc, x = x, y = y, r=r, thetadeg = thetadeg)
        self._callbacks = []
    ...
    def receive(self, chunks):
        ...
        for i in range(moduledata_cnt):
            moduledata.append( self.moduledata_nt._make(struct.unpack('>4d', chunks[moduledata_start_i+32*i:moduledata_start_i+32*i+32])) )
