...
class GUI:
    ...
    def start(self):
        ...
        try:
            ...
            self.rp.close()
            error_sound()
            print("Could not connect to an instrument")
        ...
        plt.ylabel('Signal (V)')
        pos_line, = ax.plot(time_array * 1e9, positive_array, 'b.')
        neg_line, = ax.plot(time_array * 1e9, negative_array, 'r.')
        plt.suptitle('Averaged Data', fontsize=20)
        freq_diff_text = fig.text(0.2, 0.88, f'Freq Diff: {frequency_diff}Hz', ha='center', va='bottom', size='large')
        averages_text = fig.text(0.9, 0.88, f'Avgs: {0}', ha='right', va='bottom', size='large')
        plt.grid()
        plt.show()
        ...
        while self.isRunning:
            ...
            try:
                self.sb.switch(pulse_assignments)
                self.pg.pulse(1e-3, 20, 0.08)
                np.append(self.pulse_times, time.time())
                self.sb.switch(measure_assignments)
                pos_loop_data = self.rp.get_data(trace_length, avgs_per_loop / decimation_level, decimation_level,
                                                 trigger_level)
                self.sb.switch(pulse_assignments)
                self.pg.pulse(-1e-3, 20, 0.08)
                np.append(self.pulse_times, time.time())
                self.sb.switch(measure_assignments)
                neg_loop_data = self.rp.get_data(trace_length, avgs_per_loop / decimation_level, decimation_level,
                                                 trigger_level)
            except RuntimeError:
                self.rp.close()
